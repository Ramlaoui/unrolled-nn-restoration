"""
From https://github.com/GHARBIMouna/Unrolled-Half-Quadratic
"""
...
def Fraser_Suzuki(t, a, m, sigma):
    ...
    return (
        1
        / (sigma * np.sqrt(2 * np.pi) * np.exp((np.log(2) * a**2) / 4))
        * np.exp((-1 / (2 * a**2)) * np.log(1 + a * (t - m) / sigma) ** 2)
    )
...
assert args.Dataset
datasetRoot = os.path.join("./data", args.Dataset)

if os.path.exists(datasetRoot):
    print(
        "ERROR: dataset already exists. Delete to re-create or choose a different path."
    )
    sys.exit(1)
...
training_path_Degraded2000 = os.path.join(training_path, "Degraded2000")
...
validation_path_Degraded2000 = os.path.join(validation_path, "Degraded2000")
...
MSdata = pd.read_csv("./data/MSdata.csv", encoding="mac_roman")
...
if args.Range == "Range_1":
    ...
    while training_counter < args.set_size_training:
        ...
        while (
            (Test(molecule, molecules_list) == True)
            or (
                np.amax(
                    ast.literal_eval(
                        str(MSdata["peak"][molecule_number]).replace("'", "")
                    )
                )
                >= 200
            )
        ...
        if np.max(x_true) > 30:
            ...
            np.save(os.path.join(training_path_H, f"H_tr_{training_counter}"), H)
            np.save(
                os.path.join(training_path_Groundtruth, f"x_Gr_tr_{training_counter}"),
                x_true.ravel(),
            )
            np.save(
                os.path.join(training_path_Degraded, f"x_De_tr_{training_counter}"),
                x_degraded.ravel(),
            )
            np.save(
                os.path.join(training_path_Degraded2000, f"x_De_tr_{training_counter}"),
                x_degraded_2000.ravel(),
            )
    ...
    while validation_counter < args.set_size_validation:
        ...
        if np.max(x_true) > 30:
            ...
            np.save(os.path.join(validation_path_H, f"H_va_{validation_counter}"), H)
            np.save(
                os.path.join(
                    validation_path_Groundtruth, f"x_Gr_va_{validation_counter}"
                ),
                x_true.ravel(),
            )
            np.save(
                os.path.join(validation_path_Degraded, f"x_De_va_{validation_counter}"),
                x_degraded.ravel(),
            )
            np.save(
                os.path.join(
                    validation_path_Degraded2000, f"x_De_va_{validation_counter}"
                ),
                x_degraded_2000.ravel(),
            )
    ...
    while test_counter < args.set_size_test:
        ...
        if np.max(x_true) > 30:
            ...
            np.save(
                os.path.join(test_path_Groundtruth, f"x_Gr_te_{test_counter}"),
                x_true.ravel(),
            )
            np.save(
                os.path.join(test_path_Degraded, f"x_De_te_{test_counter}"),
                x_degraded.ravel(),
            )
            np.save(
                os.path.join(test_path_Degraded2000, f"x_De_te_{test_counter}"),
                x_degraded_2000.ravel(),
            )
